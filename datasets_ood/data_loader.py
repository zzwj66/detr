from datasets_ood.cityscapes_coco import CityscapesCocoMix
import numpy
from datasets_ood.mix_ood_sampler import MixContextLoader
from torch.utils import data
from util.utils.aug import *
from util.utils.img_utils import generate_random_crop_pos, random_crop_pad_to_shape, random_mirror, normalize, random_scale, \
...
def get_mix_loader(gpu, config, proba_factor=1.):
    ...
    val_loader = MixContextLoader(dataset=test_dataset, shuffle=is_shuffle, num_workers=config.num_workers,
                                batch_size=config.batch_size, city_img_num=config.num_train_imgs,
                                coco_img_num=int(proba_factor*config.num_train_imgs), gpu_num=gpu)
